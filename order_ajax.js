    ...
    BX.Sale.Input.Utils.asMultiple = function (value) {
      if (value === undefined || value === null || value === "") {
        ...
        for (; i < length; ) {
          ...
          // === LOYALTY UI hook (appends discount row and updates total) ===
          ;(function() {
            if (!window.BX || !BX.Sale || !BX.Sale.OrderAjaxComponent) return;

            try {
              const Proto = BX.Sale.OrderAjaxComponent.constructor.prototype;
              const _ajaxSuccess = Proto.ajaxSuccess;
              const _editTotalBlock = Proto.editTotalBlock;

              function applyLoyaltyRowBadges(perItem) {
                try {
                  if (!Array.isArray(perItem) || !perItem.length) return;

                  // 1) Берём текущее «описание» корзины от компонента, чтобы связать DOM-строки с данными
                  const cmp = BX.Sale?.OrderAjaxComponent;
                  let gridRows = (cmp && cmp.result && cmp.result.GRID && cmp.result.GRID.ROWS) || [];
                  if (gridRows && !Array.isArray(gridRows) && typeof gridRows === 'object') {
                    gridRows = Object.values(gridRows);
                  }
                  const idToBarcode = {};      // basket row ID -> goodsCode
                  const idToNode = {};         // basket row ID -> DOM node
                  (gridRows || []).forEach(r => {
                    const bid = r?.data?.ID;
                    const code = r?.data?.GOODS_BARCODE || r?.data?.PROPERTY_CML2_BAR_CODE_VALUE || '';
                    if (bid) idToBarcode[String(bid)] = String(code);
                  });

                  // 2) Соберём DOM-указатели на строки корзины
                  document.querySelectorAll('[data-entity="basket-item"]').forEach(row => {
                    const id = row?.getAttribute('data-id');
                    if (id) idToNode[id] = row;
                  });

                  // 3) Для каждого ответа перешлём бейдж в нужную строку
                  perItem.forEach(item => {
                    const gc = String(item.goodsCode || '');
                    const discount = Number(item.discount || 0);
                    const bonus = Number(item.bonus || 0);
                    if (!gc) return;

                    // ищем basket row id по штрих-коду
                    const rowId = Object.keys(idToBarcode).find(bid => idToBarcode[bid] === gc);
                    if (!rowId || !idToNode[rowId]) return;

                    const row = idToNode[rowId];

                    // контейнер бейджа (переиспользуем)
                    let badge = row.querySelector('.vm-loyalty-row-badge');
                    if (!badge) {
                      badge = document.createElement('div');
                      badge.className = 'vm-loyalty-row-badge';
                      badge.style.cssText = 'margin-top:6px;font-size:12px;opacity:.9;';
                      // место вставки может отличаться — пробуем под названием товара:
                      const title = row.querySelector('.bx-soa-item-title') || row;
                      title.appendChild(badge);
                    }

                    if (discount > 0) {
                      badge.textContent = `Скидка − ${discount.toLocaleString('ru-RU')} ₽`;
                      badge.dataset.type = 'discount';
                    } else if (bonus > 0) {
                      badge.textContent = `Начислим ${bonus.toLocaleString('ru-RU')} бонусов`;
                      badge.dataset.type = 'bonus';
                    } else {
                      badge.textContent = '';
                    }
                  });

                  console.debug('[LOYALTY][UI] row badges applied', perItem);
                } catch (e) {
                  console.warn('[LOYALTY][UI] row badges error', e);
                }
              }

              // Показываем бейджи начислений (accrual) по строчкам
              function vmApplyAccrualBadges(perItem) {
                try {
                  if (!Array.isArray(perItem) || !perItem.length) return;

                  const cmp = BX.Sale?.OrderAjaxComponent;
                  let rows = (cmp && cmp.result && cmp.result.GRID && cmp.result.GRID.ROWS) || [];
                  ...
                  const idToNode = {};
                  document.querySelectorAll('[data-entity="basket-item"]').forEach(row => {
                    const id = row.getAttribute('data-id'); if (id) idToNode[id] = row;
                  });

                  console.debug('[LOYALTY][UI] vmApplyAccrualBadges input', perItem);
                  perItem.forEach(it => {
                    const gc = String(it.goodsCode || '');
                    const bonus = Number(it.bonus || 0);
                    if (!gc || bonus <= 0) return;
                    ...
                    badge.textContent = `Начислим ${bonus.toLocaleString('ru-RU')} бонусов`;
                    badge.dataset.type = 'accrual';
                  });
                } catch (e) { console.warn('[LOYALTY][UI] accrual badges error', e); }
              }

              function vmApplyAccrualTotals(accrual, isApplyWriteoff) {
                try {
                  const wrap = document.querySelector('#bx-soa-total .bx-soa-cart-total');
                  if (!wrap) return;
                  wrap.querySelectorAll('.vm-loyalty-accrual, .vm-loyalty-hint').forEach(n => n.remove());
                  console.debug('[LOYALTY][UI] vmApplyAccrualTotals input', {accrual, isApplyWriteoff});

                  if (!isApplyWriteoff && accrual && Number(accrual.totalBonus) > 0) {
                    ...
                    line.innerHTML = '<span class="bx-soa-cart-t">Начислим бонусов</span>' +
                                     '<span class="bx-soa-cart-d">+ ' + Number(accrual.totalBonus).toLocaleString('ru-RU') + '</span>';
                    // вставляем после последней строки .bx-soa-cart-total-line
                    const allLines = Array.from(wrap.querySelectorAll('.bx-soa-cart-total-line'));
                    const last = allLines.length ? allLines[allLines.length - 1] : null;
                    if (last && last.parentNode) last.parentNode.insertBefore(line, last.nextSibling);
                    else {
                      const anchor = wrap.querySelector('.bx-soa-cart-total-line-totals');
                      (anchor?.parentNode || wrap).insertBefore(line, anchor);
                    }
                  }
                } catch (e) { console.warn('[LOYALTY][UI] accrual totals error', e); }
              }

              function applyLoyaltyToTotalUI(data) {
                try {
                  const wrap = document.querySelector('#bx-soa-total .bx-soa-cart-total');
                  if (!wrap) return;
                  const totalLine = wrap.querySelector('.bx-soa-cart-total-line-totals');
                  const totalVal  = totalLine?.querySelector('.bx-soa-cart-d');
                  const old = wrap.querySelector('.bx-soa-cart-total-line.loyalty-discount');
                  if (old) old.remove();

                  const writeoff = Number(data?.writeoff || 0);
                  if (writeoff > 0) {
                    const line = document.createElement('div');
                    line.className = 'bx-soa-cart-total-line loyalty-discount';
                    line.innerHTML = '<span class="bx-soa-cart-t">Списано бонусов</span>' +
                                     '<span class="bx-soa-cart-d">− ' + writeoff.toLocaleString('ru-RU') + ' ₽</span>';
                    // вставляем после последней строки .bx-soa-cart-total-line
                    ...
                    else if (totalLine?.parentNode) totalLine.parentNode.insertBefore(line, totalLine);
                  }
                  if (totalVal && data?.total != null) {
                    totalVal.textContent = Number(data.total).toLocaleString('ru-RU') + ' ₽';
                  }

                  // НОВОЕ: рисуем бейджи по строчным начислениям/скидкам и показываем итоги начисления
                  const isApply = !!document.querySelector('#loyalty-toggle:checked');

                  // старые perItem — скидки/бонусы (backwards-compatible)
                  if (Array.isArray(data?.perItem) && data.perItem.length) {
                    applyLoyaltyRowBadges(data.perItem);
                  }

                  // новый блок accrual: показываем в итогах и по строчкам, если списание выключено
                  if (data?.accrual) {
                    vmApplyAccrualTotals(data.accrual, isApply);
                    if (Array.isArray(data.accrual.perItem)) {
                      if (!isApply) vmApplyAccrualBadges(data.accrual.perItem);
                      else document.querySelectorAll('.vm-loyalty-row-badge[data-type="accrual"]').forEach(n => n.remove());
                    }
                  }
                } catch(e){ console.warn('[LOYALTY][UI] apply error', e); }
              }

              Proto.ajaxSuccess = function(response, request) {
                try {
                  if (response && (response.writeoff !== undefined || response.total !== undefined)) {
                    window.__LOYALTY_LAST_RESPONSE = response;
                    applyLoyaltyToTotalUI(response);
                  }
                } catch(e){ console.warn('[LOYALTY][ajaxSuccess] hook error', e); }
                return _ajaxSuccess.apply(this, arguments);
              };

              Proto.editTotalBlock = function() {
                _editTotalBlock.apply(this, arguments);
                if (window.__LOYALTY_LAST_RESPONSE && window.__LOYALTY_LAST_RESPONSE.ok) {
                  applyLoyaltyToTotalUI(window.__LOYALTY_LAST_RESPONSE);
                }
              };
            } catch(e){ console.warn('[LOYALTY][hook install error]', e); }
          })();
        }
