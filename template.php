...
// Ensure BX core is initialized (ajax + bitrix_sessid available)
...
<?php /* === LOYALTY BLOCK (BEGIN) === */ ?>
									<?php if (false): ?>
									<div class="vm-loyalty" id="vm-loyalty" data-init="0">
									  <div class="vm-loyalty__head">
										<div class="vm-loyalty__title">Бонусная программа</div>
										<label class="vm-switch">
										  <input type="checkbox" id="loyalty-toggle" />
										  <span class="vm-switch__slider"></span>
										  <span class="vm-switch__label">Списывать бонусы</span>
										</label>
									  </div>

									  <div id="loyalty-cards" class="vm-cards">
										<div class="vm-cards__empty">Ищем привязанные карты…</div>
									  </div>

									  <div class="vm-loyalty__controls">
										<button class="vm-btn vm-btn--apply" id="loyalty-apply" disabled>Применить</button>
										<button class="vm-btn vm-btn--clear" id="loyalty-clear">Сбросить</button>
									  </div>

									  <div class="vm-loyalty__status" id="loyalty-status"></div>
									</div>
									<?php endif; ?>
									<?php /* === LOYALTY BLOCK (END) === */ ?>
	...
	<script>
		...
		// VM loyalty config: forced test region (266) and sessid
		window.VM_LOYALTY = {
  regionId: 266, // заглушка Улан-Удэ
  regionMap: {
    266: { shop: 6, terminalId: 71 }, // Улан-Удэ
  },
  sessId: '<?= bitrix_sessid() ?>'
        ...
        // Legacy payload helpers (for exact old SET request body)
        window.LEGACY_BASKET_ITEMS = <?=CUtil::PhpToJSObject($arResult['BASKET_ITEMS'])?>;
        window.LEGACY_ORDER_PRICE  = <?=json_encode((float)$arResult['JS_DATA']['TOTAL']['ORDER_PRICE'])?>;
        window.LEGACY_REGION_ID = 266; // temporary stub
        window.BX_SESSID = '<?=bitrix_sessid()?>';
	</script>
	<script>
		<?
